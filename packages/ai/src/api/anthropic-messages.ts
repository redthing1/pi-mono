...
function shouldUseServerSideFallbackBeta(
	model: Model<"anthropic-messages">,
	allowModelFallbacks: boolean | undefined,
): boolean {
	return allowModelFallbacks !== false && (model.compat?.allowedFallbackModels?.length ?? 0) > 0;
}
		...
		try {
			...
			if (options?.client) {
				...
				const created = createClient(
					...
					shouldUseServerSideFallbackBeta(model, options?.allowModelFallbacks),
					options?.headers,
					options?.fetch,
...
function buildParams(
	...
	const allowedFallbackModels =
		options?.allowModelFallbacks === false ? undefined : model.compat?.allowedFallbackModels;
