			...
			const finishBlock = (block: StreamingBlock) => {
				...
				if (block.type === "text") {
					...
					if (block.customInput) {
						...
						try {
							block.arguments = parseJsonWithRepair(block.partialArgs?.trim() ? block.partialArgs : "{}");
						} catch {
							block.arguments = {};
							block.argumentsParseError = true;
						}
					}
...
function buildParams(
	...
	if (compat.vllmPriority !== undefined) {
		(params as typeof params & { priority?: number }).priority = compat.vllmPriority;
	}
