...
- Fixed interactive and RPC startup contacting provider model catalogs without an explicit refresh request.
- Fixed remote-ZDR requests allowing provider-side substitution to an unapproved fallback model.
- Fixed provider routing and cache affinity exposing the durable local session ID; active runtimes now use an ephemeral opaque identifier.
