...
To share extensions as local pi packages, see [packages.md](packages.md).
...
Extensions can use dependencies that are already present beside the extension (or in a parent directory). Imports from `node_modules/` resolve automatically, but Pi never installs or fetches them.

For a local pi package, vendor or bundle every runtime dependency before loading it. Pi never runs Bun, npm, pnpm, or Yarn inside a package, and registry, Git, and URL package sources are disabled in this fork.
...
**Package with dependencies** - for extensions that need package dependencies:
