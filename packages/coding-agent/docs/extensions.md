...
For distributed git pi packages installed with `pi install`, runtime deps must be in `dependencies`. Package installation uses Bun production installs with `--ignore-scripts`, so `devDependencies` are not available at runtime. Registry package installs are disabled in this fork.
