...
export class ModelRuntime implements Models {
	...
	/** Whether a model is explicitly approved for zero-data-retention use. */
	isZdrModel(model: Model<Api>): boolean {
		const compat = model.compat as { openRouterRouting?: { zdr?: boolean } } | undefined;
		return model.zdr === true || compat?.openRouterRouting?.zdr === true;
	}
