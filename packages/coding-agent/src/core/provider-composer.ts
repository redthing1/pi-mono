...
function modelFromJson(
	...
	return {
		...
		provider: providerId,
		baseUrl,
		zdr: definition.zdr ?? providerConfig.zdr ?? defaults?.zdr,
...
function applyModelsJson(
	...
	if (
		...
		throw new Error(
			`Provider ${providerId}: must specify "baseUrl", "headers", "compat", "zdr", "modelOverrides", or "models".`,
		);
	}
...
function applyExtension(
	...
	if (!config.models) {
		return config.baseUrl || config.zdr !== undefined
			? models.map((model) => ({
					...model,
	...
	return config.models.map((definition) => {
		...
		return {
			...
			provider: providerId,
			baseUrl,
			zdr: definition.zdr ?? config.zdr ?? defaults?.zdr,
			headers: undefined,
		};
	});
}
