...
export class SettingsSelectorComponent extends Container {
	...
	constructor(config: SettingsConfig, callbacks: SettingsCallbacks) {
		...
		// Compact exploration toggle (insert after terminal-progress)
		const terminalProgressIndex = items.findIndex((item) => item.id === "terminal-progress");
		items.splice(terminalProgressIndex + 1, 0, {
			id: "compact-exploration",
			label: "Compact exploration",
			description: "Compact read/search/list exploration output in interactive mode",
			currentValue: config.compactExploration ? "true" : "false",
			values: ["true", "false"],
		});
			...
			(id, newValue) => {
				switch (id) {
					...
					case "terminal-progress":
						...
						break;
					case "compact-exploration":
						callbacks.onCompactExplorationChange(newValue === "true");
						break;
				}
			},
			callbacks.onCancel,
